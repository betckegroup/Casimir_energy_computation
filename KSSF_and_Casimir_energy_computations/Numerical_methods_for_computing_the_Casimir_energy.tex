...
\begin{align*}
    ...
    L_{\text{loc}}^{2}(\Omega^{+}) &:= \left\{f:\Omega^{+}\rightarrow\mathbb{C},\ f \text{ is Lebesgue measurable and} \int_{K}|f|^{2} < \infty, \ \text{for any compact}\ K \subset \overline{\Omega^{+}} \right\}
\end{align*}
and note that the subscript ``loc'' can be removed if the domain is bounded (i.e. $L_{\text{loc}}^{2}(\Omega^{-}) = L^{2}(\Omega^{-})$). We denote by $H_{\text{loc}}^{s}(\Omega^{\pm})$ the standard local Sobolev spaces associated with the Lipschitz domains. In particular, for integer $s\geq0$ we have
\begin{align*}
...
$\gamma_{D}^{\pm}:  H_{\text{loc}}^1(\Omega) \to H^{1/2}(\Gamma) $. For the purposes of this paper it is sufficient to understand $H^{1/2}(\Gamma)$ as range space of the trace operator on $H_{\text{loc}}^1(\Omega)$. We will also need the space $H^{-1/2}(\Gamma)$, which is the dual space of $H^{1/2}(\Gamma)$ with $L^2(\Gamma)$ as pivot space.
...
\begin{theorem}\cite{MR4484208}
    ...
    $V_{k}\tilde{V}_{k}^{-1} - I$, with $I$ the identity operator, is trace-class. The function
    \begin{align*}
        \Xi(k) = \log\det\left(V_{k}\tilde{V}_{k}^{-1}\right)
    \end{align*}
    is well defined using the Fredholm determinant
   $\det(V_{k}\tilde{V}_{k}^{-1})$ 
    ...
    and fixing the branch of the logarithm so that $\Xi$ is continuous in the upper half plane and converges to zero as the imaginary part of $k$ goes to $\infty$.
   By taking $m = 0$ and $s = 1$ in \eqref{trace formula in terms of the boundary op}, we have the formula 
    ...
    \end{align}
    for the trace of the trace-class operator $\Delta^{\frac{1}{2}} + (N - 1)\Delta_{0}^{\frac{1}{2}} - \sum_{i = 1}^{N}\Delta_{j}^{\frac{1}{2}}$.
\end{theorem}
...
\end{align}
This formula for the Casimir energy coincides with that proposed by Johnson in \cite{reid2009efficient} who uses a non-rigorous path integral argument and the surface fluctuation mechanism of the Casimir effect for its derivation.
