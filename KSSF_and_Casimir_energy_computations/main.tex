...
\begin{abstract}
    Computing the Casimir force and energy between objects is a classical problem of quantum theory going back to the 1940s and remains a challenge to this day. A recent promising approach is the formulation in terms of log determinants of certain boundary integral operators, which can be numerically evaluated efficiently and woks well for smaller problems sizes. In this paper we present an introduction to the Casimir energy and the Krein spectral shift function, using notation more familiar to numerical researchers than that in the corresponding physics literataure. This is accompanied by numerical examples of boundary integral operator discretisations for these problems. We then introduce an improved projection method that allows us to compute the Casimir energy also for large-scale problems. The focus in this paper is on the Casimir energy and Krein spectral shift function for scalar fields. But we will comment on ongoing work on large-scale methods for the full vectorial field.
   \end{abstract}
...
\input{Introduction_and_Settings.tex}
...
\input{Improvements_of_the_computational_process.tex}
